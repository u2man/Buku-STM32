...
void BSP_LCD_Debug(uint8_t* DebugBuffer){
	...
	if(DebugCounter<20){
		memset(DebugLineData[DebugCounter],' ',45);
		memcpy(DebugLineData[DebugCounter++],DebugBuffer, strlen((char*)DebugBuffer));
	}
	else{
	  DebugCounter=20;
	  for(line=0;line<19;line++){
		  memset(DebugLineData[line],' ',45);
		  memcpy(DebugLineData[line],DebugLineData[line+1],45);
	  }
	  memset(DebugLineData[19],' ',45);
	  memcpy(DebugLineData[19],DebugBuffer, strlen((char*)DebugBuffer));

	}

	printf("%s\r\n",DebugLineData[DebugCounter-1]);
	for(i=0;i<20;i++){
		BSP_LCD_DisplayStringAtLine(i, DebugLineData[i]);
